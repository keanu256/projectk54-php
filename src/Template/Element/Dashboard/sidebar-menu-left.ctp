...
<div class="br-sideleft overflow-y-auto">
    ...
    <?php if($this->request->session()->read('Auth.User.isadmin')==1): ?>
    <label class="sidebar-label pd-x-15 mg-t-25 mg-b-20 tx-info op-9" style="font-size: 1.5vh;">ADMIN PANEL</label>
    <div class="br-sideleft-menu">
        <?php foreach ($layouts['sidebar_menu_left'] as $key => $value): ?>
        <?php if($value->type == 1): ?>
        <a menu-t="<?= empty($value->sidebar_left_child)? 0:1; ?>" href="#" class="br-menu-link pa-link <?= ($key == 0)? 'active':''; ?>" function-id="<?= $value->functionid ?>">
